...
def json_stringify_rx(rx: dict) -> str:
    ...
    for med in rx.get("medicines", []):
        meds_list.append(
            f'{{"name":"{med.get("name", "")}",'
            f'"interval":"{med.get("interval", "")}",'
            f'"morning":{str(med.get("morning", False)).lower()},'
            f'"afternoon":{str(med.get("afternoon", False)).lower()},'
            f'"evening":{str(med.get("evening", False)).lower()},'
            f'"night":{str(med.get("night", False)).lower()},'
            f'"beforeFood":{str(med.get("beforeFood", False)).lower()},'
            f'"afterFood":{str(med.get("afterFood", False)).lower()},'
            f'"customInstruction":"{med.get("customInstruction", "")}"}}'
        )
