...
class _PharmacyVerificationScreenState extends State<PharmacyVerificationScreen> {
  ...
  Widget build(BuildContext context) {
      ...
      content = Column(
        children: [
          NeonCard(
            ...
            child: ListTile(
              leading: const Icon(Icons.error_outline, color: Color(0xFFEF4444), size: 40),
              title: Text('Verification Failed: $_verdict', style: const TextStyle(fontWeight: FontWeight.bold)),
              subtitle: Text(
                _errorMsg ?? 'Prescription verification failed. Signature is invalid or single-use token already burned.',
        ...
        children: [
          NeonCard(
            ...
            child: Column(
              children: [
                ListTile(
                  leading: const Icon(Icons.verified_user_rounded, color: Color(0xFF10B981), size: 36),
                  title: const Text('Signature Verified', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  subtitle: Text(
          ...
          Text('Prescription Details', style: GoogleFonts.sora(fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
          NeonCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Prescription ID: ${rx['id']}', style: GoogleFonts.jetBrainsMono(fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 8),
                Text('Patient: ${rx['patientName']}'),
                Text('Diagnosis: ${rx['disease']}'),
                Text('Clinician: ${rx['doctorName']} (NPI: ${rx['doctorSignId']})'),
                ...
                ...medsList.map((m) {
                  ...
                  final timingStr = timings.isEmpty ? m['interval'] : timings.join(', ');
                  final foodStr = m['beforeFood'] == true ? ' (Before Food)' : (m['afterFood'] == true ? ' (After Food)' : '');
                  final custom = m['customInstruction'] != null && m['customInstruction'].toString().isNotEmpty ? " - ${m['customInstruction']}" : "";
                  return Padding(
          ...
          SizedBox(
            ...
            child: GlassmorphicButton(
              ...
              child: const Text('Proceed to Dispensation Desk', style: TextStyle(fontWeight: FontWeight.bold)),
            ),
          ),
        ],
      );
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Cryptographic Verification', style: TextStyle(fontFamily: 'Sora')),
      ),
