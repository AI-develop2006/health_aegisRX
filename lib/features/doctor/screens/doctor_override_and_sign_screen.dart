...
class _DoctorOverrideAndSignScreenState extends State<DoctorOverrideAndSignScreen> {
  ...
  void _signAndCommit() {
    ...
    // Format medicines list for the Pydantic PrescriptionCreate model
    final List<Map<String, dynamic>> medicinesPayload = widget.medications.map((med) {
      return {
    ...
    // Unique random prescription ID
    final randomRxId = "RX-${1000 + Random().nextInt(9000)}";

    final rxData = {
      'id': randomRxId,
      'doctorName': appState.doctorLicense != null ? "Dr. ${appState.doctorLicense}" : "Dr. Alexander Vance",
      'hospitalName': appState.doctorHospital ?? "Metropolitan Hospital Centre",
    ...
    appState.createPrescription(rxData).then((res) {
      ...
      if (res['error'] != null || res['detail'] != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Error: ${res['error'] ?? res['detail'] ?? 'Prescription failed'}")),
        );
        return;
      }

      final txHash = res['onchain_tx_hash'] ?? _generateMockHash();
            ...
            actions: [
              TextButton(
                onPressed: () {
                  ...
                  Navigator.pop(context); // close sign screen
